...
def calc_N_from_distribution(V, N_bins=70):
    ...
    fig, ax = plt.subplots()
    counts, bins, patches = ax.hist(V, bins=N_bins, density=True)
    ...
    ax.plot(XX, gamma_pdf(XX, N, I_0), 'r-', label=f'fit to gamma: N={N:.3f}$\pm${N_err:.2f}, $ I_0 $={I_0:.1f}$\pm${I_0_err:.1f}')
